...
pub fn cmd_verify(hid: &SoloHid) -> Result<()> {
    ...
    let pin_uv_auth: Option<Vec<u8>> = if credential::get_info_client_pin_set(hid)? {
...
pub fn cmd_change_pin(hid: &SoloHid) -> Result<()> {
    ...
    let change_pin_cbor = Value::Map(vec![
        ...
        (Value::Integer(0x04u64.into()), Value::Bytes(pin_uv_auth_param.to_vec())),   // pinUvAuthParam (16 bytes)
        (Value::Integer(0x05u64.into()), Value::Bytes(new_pin_enc.to_vec())),         // newPinEnc (64 bytes)
        (Value::Integer(0x06u64.into()), Value::Bytes(pin_hash_enc.to_vec())),        // pinHashEnc (16 bytes)
    ]);
    ...
    if change_pin_response.is_empty() {
        return Err(SoloError::DeviceError("Empty response from changePin".into()));
    }
